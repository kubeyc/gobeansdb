...
type HStore struct {
	buckets   []*Bucket // 管理bitcask实例
	gcMgr     *GCMgr    // gc
	htree     *HTree    // 元数据 hash tree
...
func NewHStore() (store *HStore, err error) {
	...
	// 初始化bucket
	for i := 0; i < Conf.NumBucket; i++ {
		need := Conf.BucketsStat[i] > 0 // > 0 可能处于 BUCKET_STAT_NOT_EMPTY 与 BUCKET_STAT_READY 两种状态
		found := store.buckets[i].State >= BUCKET_STAT_NOT_EMPTY
		if need {
			if !found {
				err = store.allocBucket(i) // 分配bucket，仅创建文件夹
				if err != nil {
					return
				}
